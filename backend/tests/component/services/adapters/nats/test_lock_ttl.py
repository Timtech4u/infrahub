...
async def test_init_locks_carry_configured_ttl(nats: dict[int, int] | None) -> None:
    if config.SETTINGS.cache.driver != config.CacheDriver.NATS:
        pytest.skip("Must use NATS to run this test")
    ...
    assert registry.initialization().ttl == expected_ttl
    assert registry.get(name=GLOBAL_TASKMGR_INIT_LOCK).ttl == expected_ttl
